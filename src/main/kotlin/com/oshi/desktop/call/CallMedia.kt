...
class CallMediaLeg internal constructor(
    ...
    fun sendMedia(sealed: ByteArray): Boolean {
        ...
        if (mirror != null && mirror != primary && sendOn(mirror, sealed)) sent = true
        return sent
    }

    private fun sendOn(carrier: TxCarrierPolicy.Carrier, sealed: ByteArray): Boolean = when (carrier) {
                ...
                CallMediaLeg(
                    ...
                    audioFor = { send ->
                        CallAudioSession(spec.sessionKey, spec.nonceSalt, spec.isCaller, send)
                            .also { it.useWbAdpcm = spec.wbAdpcm; it.log = log }
                    },

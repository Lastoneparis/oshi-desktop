...
| 0.27 | **LoRa / Meshtastic transport** | `LoRaTransport.{swift,kt}`, `MeshtasticManager`, `LoRaEnvelope.kt`, `LoRaNodeIdentityMap`, `LoRaPacketParser` | ⬜ | **Missing from every earlier version of this ledger, and it is not small**: 8 Swift files and 12 Kotlin files. A real third transport, off-grid and with no internet — which is the one thing the desktop cannot claim today (row 0.16). Its defining constraint is the MTU: a Meshtastic frame budget is ~237 bytes, so OSHI fragments into ≤180-byte chunks behind an 8-byte header (`msgId`, `seq`, `total`) and reassembles. Open questions the implementation must answer from the source, not from habit: whether `loraNodeNum` → identity is AUTHENTICATED or spoofable, what happens to plain Meshtastic text from a non-OSHI device (`handleLoRaInteropText`), and whether a JDK can reach the hardware at all without a native serial dependency |
